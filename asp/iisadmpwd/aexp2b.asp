...
<%if Request.Form("cancel") <> "" then
	Response.Redirect(NoAngles(Request.QueryString)) 
end if
%>
...
<form method="POST" action="https://<%=Request.ServerVariables("SERVER_NAME")%>/iisadmpwd/achg.asp?<%= NoAngles(Request.QueryString)%>">

<table>
<tr>
<td>Domain</td><td><input type="text" name="domain" value="
<%if Request.Form("domain") <> "" then %>
	<%= NoAngles(Request.Form("domain"))%>
<%else%>
	<%
